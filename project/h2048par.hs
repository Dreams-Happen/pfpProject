...
parMaximize grid a b maxDepth
  ...
  | otherwise = chooseGrid (chooseGridIndex utilities maximum 0) (grids) 
...
maxUtility :: [Integer] -> Integer
maxUtility utilities = maximum utilities 

chooseGridIndex :: [Integer] -> Integer -> Int -> Int 
chooseGridIndex (x:xs) maxUtility starter
                | x == maxUtility = starter
                | otherwise = chooseGridIndex xs maxUtility (starter + 1) 
